        ...
        private static void Run(Menu menuInstance, Log CLILoggingInstance, Settings settingsInstance)
        {
            ...
            while (running)
            {
                ...
                int opt = inputHandel.GetOption("Please select an option to continue:",
                    new[]
                    {
                        "Process New Image Into Map Data File", "Recall Map From Data File", "Settings", "Exit Program", "", "Testing Menu (To Be Removed For Final Solution)"
                    });
                ...
                switch (opt)
                {
                    ...
                    case 2:
                        ...
                        catch (Exception ex)
                        {
                            ...
                            menuInstance.Error($"Your settings file is corrupt, please delete the 'settings.conf' file and restart. The program will now exit. Details: {(ex.Message != null ? ex.Message : "")}");
