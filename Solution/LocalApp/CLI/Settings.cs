...
namespace LocalApp.CLI
{
    public class Settings
    {
        ...
        public void Change(string setting, bool value)
        {
            if (!UserSettings.ContainsKey(setting)) throw new SettingsException($"Setting {setting} does not exist, if this problem persists delete settings.conf and restart the program.");
            UserSettings[setting] = (value.ToString().ToLower(), typeof(bool));
        }

        public void Change(string setting, int value)
        {
            if (!UserSettings.ContainsKey(setting)) throw new SettingsException($"Setting {setting} does not exist, if this problem persists delete settings.conf and restart the program.");
            UserSettings[setting] = (value.ToString(), typeof(int));
        }

        public void Change(string setting, double value)
        {
            if (!UserSettings.ContainsKey(setting)) throw new SettingsException($"Setting {setting} does not exist, if this problem persists delete settings.conf and restart the program.");
        ...
        public bool TryChange(string setting, bool value)
        {
        ...
        public bool TryChange(string setting, int value)
        {
            if (!UserSettings.ContainsKey(setting)) return false;
            UserSettings[setting] = (value.ToString(), typeof(int));
        ...
        public bool TryChange(string setting, double value)
        {
            if (!UserSettings.ContainsKey(setting)) return false;
            UserSettings[setting] = (value.ToString(), typeof(double));
        ...
        public bool TryChange(string setting, string value)
        {
            if (!UserSettings.ContainsKey(setting)) return false;
            UserSettings[setting] = (value.ToString(), typeof(string));
